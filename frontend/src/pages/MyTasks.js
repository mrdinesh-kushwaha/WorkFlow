...
const MyTasks = () => {
  ...
  useEffect(() => {
    taskAPI.myTasks().then(r => setTasks(r.data)).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div style={{ padding:32, maxWidth:900 }}>
      <div style={{ marginBottom:28 }}>
        <h1 style={{ color:'#f1f5f9', fontSize:24, fontWeight:700, margin:0, letterSpacing:'-0.5px' }}>My Tasks</h1>
        <p style={{ color:'#64748b', fontSize:14, marginTop:4 }}>{tasks.length} task{tasks.length!==1?'s':''} assigned to you</p>
      </div>
      ...
      {overdue.length > 0 && (
        <div style={{ background:'#ef444410', border:'1px solid #ef444430', borderRadius:10, padding:'12px 16px', marginBottom:20, display:'flex', alignItems:'center', gap:10 }}>
          ...
          <span style={{ color:'#ef4444', fontSize:14, fontWeight:500 }}>{overdue.length} overdue task{overdue.length!==1?'s':''} need attention</span>
        </div>
      )}
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:12 }}>
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ display:'flex', gap:10, fontSize:12, color:'#64748b', alignItems:'center', flexWrap:'wrap' }}>
                    ...
                    <span style={{ padding:'2px 7px', borderRadius:20, fontSize:11, background:`${priorityColors[task.priority]}20`, color:priorityColors[task.priority] }}>{task.priority}</span>
