...
const Login = () => {
    ...
    useEffect(() => {
        ...
        if (error === 'user_exists') {
            toast.error('Your data does not exist in our system. Please sign up.', {
                id: 'user-not-registered-error'
            });
                ...
                <div style={{ background:'#1e293b', borderRadius:16, padding:32, border:'1px solid #334155' }}>
                        ...
                        {[
                            ...
                            <div key={key} style={{ marginBottom:20 }}>
                                ...
                                <input type={type} value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })}
                                       ...
                                       style={{ width:'100%', padding:'12px 14px', background:'#0f172a', border:'1px solid #334155', borderRadius:8, color:'#f1f5f9', fontSize:14, outline:'none', boxSizing:'border-box', transition:'border-color 0.15s' }}
                                       onFocus={e => e.target.style.borderColor='#6366f1'} onBlur={e => e.target.style.borderColor='#334155'} />
                            </div>
                        ))}
                        <button type="submit" disabled={loading} style={{ width:'100%', padding:'14px', background:'#238636', border:'none', borderRadius:8, color:'white', fontSize:16, fontWeight:700, cursor:'pointer', opacity: loading ? 0.7 : 1, marginTop:4 }}>
                    ...
                    <div style={{
                        ...
                        <button
                            type="button"
                            onClick={() => {
                                document.cookie = `TF_AUTH_MODE=LOGIN; path=/`;
